...
packageMeta:
  ...
  tag: [build-tools]
pageData:
  title: "API: cleanTypesBuildArtifacts"
...
- **Source region comments** *(e.g. `// #region`, `// #endregion`, `//#region`, `//#endregion`)*  
  ...
  ➔ See option: [`removeRegion`](#remove-region).
...
<Callout type="info" title={<strong>Info</strong>}> 
  Pattern resolution behavior can be customized via <CustomNextLink href='#pattern-options'>`options.patternOptions`</CustomNextLink> (e.g. hidden files, case sensitivity, concurrency, and ignore rules).
</Callout>
