...
import type { TypeSimplifierOptions } from "./get-simple-form";
...
export function createGenerator(options: GeneratorOptions = {}) {
  ...
  function getSourceFile(
    ...
    const pkgAndVersion =
      isNonEmptyString(pkg) && isNonEmptyString(version)
        ? `${pkg}_(${version})-`
        : "";
    ...
    const finalSourceFile = sourceFile
      ? sourceFile
  ...
  return {
    async generateDocumentation(
      ...
      if (cache) {
        cacheKey = generateHash(
          `${file.path}:${name}:${content}:${packageVersion}`
        );
...
async function getDocEntry(
  ...
  let typeDecsLink: string | undefined;
  const linkTag = tags.find((tag) => tag.name === "link");
  if (linkTag) {
    const match =
      /\[(?<label>.+?)\]\((?<url>(https?:\/\/|\/|#|\.\/)[^)]+)\)/.exec(
        linkTag.text
      );
    const url = match?.groups?.url?.trim();
    if (url) typeDecsLink = url;
  }
