...
import { type ParameterTag, parseTags } from "./parse-tags";
import { createGenerator, type DocEntry, type Generator } from "./base";
...
async function buildTypeProp(
  ...
  async function onItem(entry: DocEntry) {
    ...
    if (entry.typeHref)
      node.addExpressionNode(
        "typeDescriptionLink",
        valueToEstree(entry.typeHref)
      );

    if (tags.default)
      node.addJsxProperty(
        "default",
        await renderer.renderTypeToHast(tags.default)
      );

    if (tags.returns)
      node.addJsxProperty(
        "returns",
        await renderer.renderMarkdownToHast(tags.returns)
      );
  ...
  async function onParam(param: ParameterTag) {
    ...
    if (param.description)
      node.addJsxProperty(
        "description",
        await renderer.renderMarkdownToHast(param.description)
      );
