...
import { cnVer3 } from "@/tailwind/class-names/cn";

describe("cnVer3 (default)", () => {
  it("should merge basic Tailwind classes", () => {
    const result = cnVer3("p-2 p-4");
    expect(result).toBe("p-4");
  });

  it("should merge cx arrays", () => {
    const result = cnVer3(["p-2", "p-4"], ["m-1", "m-3"]);
  ...
  it("should merge nested arrays and falsy values", () => {
    const result = cnVer3(["p-2", ["p-4", undefined]], null, false, "p-6");
