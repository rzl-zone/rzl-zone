...
export class PageTreeBuilder {
  ...
  private readonly ownerMap = new Map<
    PageTree.Node,
    { owner: string; priority: number }
  >();
  private _nextId = 0;
  ...
  constructor(
    input:
      | ContentStorage
      | [locale: string, storages: Record<string, ContentStorage>],
  ...
  folder(folderPath: string): PageTree.Folder | undefined {
    ...
    const metaPath = this.resolveFlattenPath(
      joinPath(folderPath, "meta"),
      "meta"
    );
    const indexPath = this.resolveFlattenPath(
