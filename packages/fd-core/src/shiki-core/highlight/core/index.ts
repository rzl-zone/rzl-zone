...
export async function highlightHast(
  ...
  if ("theme" in resolved) {
    ...
    themesToLoad = Object.values(resolved.themes).filter(
      (v) => v !== undefined
    );
...
async function loadMissingTheme(
  ...
  if (missingThemes.length > 0)
    await highlighter.loadTheme(...(missingThemes as never[]));
}
...
async function loadMissingLanguage(
  ...
  if (missingLangs.length > 0)
    await highlighter.loadLanguage(...(missingLangs as never[]));
}
