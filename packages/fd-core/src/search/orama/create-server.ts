...
import { searchSimple } from "./simple";
import { searchAdvanced } from "./advanced";
import { createEndpoint, defaultReadOptions } from "../server/endpoint";

type OramaInput = Parameters<typeof create>[0];

interface SharedOptions extends Pick<
  OramaInput,
  "sort" | "components" | "plugins"
> {
  language?: string;
...
): SearchServer<OramaQueryOptions> {
...
export function createSearchAPI(
  ...
  return toAPI(
    args[0] === "simple"
      ? initSimpleSearch(args[1])
      : initAdvancedSearch(args[1])
  );
}
...
function getTokenizer(
  locale: string
): { language: string } | { tokenizer: Tokenizer } {
  return {
    language:
...
export function createI18nSearchAPI(
  ...[type, options]:
    | ["simple", I18nSimpleOptions]
    | ["advanced", I18nAdvancedOptions]
): SearchAPI<OramaQueryOptions> {
...
> {
  ...
  buildIndex?: (
    page: Page<ResolvePage<C["source"]["pageData"]>>
      ...
      async indexes() {
        const indexes = source.getLanguages().flatMap((entry) => {
          return entry.pages.map(async (page) => {
            ...
            const index = await buildIndex(page as any);
            return {
              ...index,
              breadcrumbs:
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                index.breadcrumbs ?? buildBreadcrumbs(source, page as any),
      ...
      async indexes() {
        const indexes = source.getPages().map(async (page) => {
          ...
          return {
            ...index,
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            breadcrumbs: buildBreadcrumbs(source, page as any)
          };
        });
...
function toAPI(
  server: SearchServer<OramaQueryOptions>
): SearchAPI<OramaQueryOptions> {
